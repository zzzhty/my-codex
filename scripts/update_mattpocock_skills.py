...
def run_cachebuster(plugin_root: Path) -> None:
    script = codex_home() / "skills" / ".system" / "plugin-creator" / "scripts" / "update_plugin_cachebuster.py"
...
def validate(plugin_root: Path) -> None:
    ...
    validator = Path(
        os.environ.get(
            "PLUGIN_VALIDATOR",
            str(home / "skills" / ".system" / "plugin-creator" / "scripts" / "validate_plugin.py"),
        )
    )
    quick_validator = home / "skills" / ".system" / "skill-creator" / "scripts" / "quick_validate.py"
