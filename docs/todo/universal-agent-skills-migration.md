...
Continuation contract: Read this file, root `AGENTS.md`, current `main`, the active M5 branch or PR, and the newest user request before acting. M0 through M4 are complete; M5 is `In Progress` for a formal identity-contract evolution after candidate universal discovery preserved exact-once source loading and exposed the same native plugin-qualified Codex invocation identities as the plugin baseline. On 2026-08-21 the user explicitly accepted that qualified surface and superseded the neutral-layout direction. The current Mac remains safely restored to the plugin profile with repaired repo-owned hooks. Preserve the existing physical source layout, protected backup, bare catalog-name mapping, qualified invocation identities, frozen slimming baseline, user-owned installation state, and single-active-discovery-path invariant. Do not retry live cutover until the evolved oracle and cutover artifacts pass independent review and the user explicitly requests execution under this evolved contract. Do not depend on prior chat, deleted prompts, temporary branches, or unpublished artifacts.
...
Open decisions: `None for the identity contract. The 2026-08-21 user decision accepts native qualified Codex invocation identities and rejects a neutral-layout workaround. Live M5 retry remains paused only until the evolved oracle and cutover artifacts pass independent review and the user explicitly resumes execution; that is an execution boundary, not an unresolved product choice.`
...
Rough elapsed-time estimate: `1-3 days`

Basis or blocker: `2026-08-21 M5 contract-evolution rebaseline is 1-3 days from the current milestone to Closed. M1-M4 are merged, no source relocation is needed, and the remaining critical path is the deterministic qualified-identity oracle and cutover-artifact update, focused validation, independent cutover review, one controlled candidate/rollback/final run, Draft PR completion, merge, and Close. The repository currently has no CI workflow, and the separately scoped cleanup observation window is excluded. The range excludes prolonged credential, machine-access, or upstream-tool outages.`
...
- Current safe state is the single active plugin discovery profile with the repo-owned hooks repaired and healthy; `/Users/max/.agents/skills` is inactive and empty. No retained backup, Watcher state, unrelated plugin, marketplace, config, or user entry was deleted. The formal identity evolution is now authorized, but live retry remains paused until its artifacts and review pass and the user explicitly resumes execution.
...
Checkpoint component: Pending because the evolved identity oracle, independent cutover review, final universal run, and M5 completion evidence are not yet complete.

Checkpoint revision: `625e3bdcbfe64085c7ddbeec2601c5c5e0077ff1` is the reviewed pre-apply source; `ca23a10b4195db852b31a156b530e9ce6d484911` records the historical blocked-state evidence in Draft PR #10. The evolving M5 checkpoint remains Pending and unmerged.
...
Authorization and review: `The user authorized formal identity-contract evolution on 2026-08-21 and then paused live execution. Read-only inventory and planning updates may continue. The exact M5 apply and rollback operations recorded by the completed preflight become executable only after the evolved oracle and artifacts pass independent cutover review and the user explicitly resumes execution under this contract. Any broader mutation is a runtime hard stop.`
...
Before marking the milestone Done, run its minimal falsifying checks, required broader validation, independent read-only Contract review, docs synchronization, and checkpoint evidence. Perform M5 only after its evolved identity/source-locator oracle passes review and the user explicitly resumes execution, and only inside its frozen apply and rollback boundary. Do not delete retained plugin, marketplace, cache, hook-backup, or Watcher state; cleanup belongs to a later independent goal.
```
