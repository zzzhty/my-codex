...
Continuation contract: Read this file, root `AGENTS.md`, current `main`, the active M5 branch or PR, and the newest user request before acting. M0 through M4 are complete; M5 is `In Progress` for a formal identity-contract evolution after candidate universal discovery preserved exact-once source loading and exposed the same native plugin-qualified Codex invocation identities as the plugin baseline. On 2026-08-21 the user explicitly accepted that qualified surface, superseded the neutral-layout direction, and then explicitly requested execution under the evolved contract after the qualified oracle and cutover artifacts completed independent review. The current Mac remains safely restored to the plugin profile with repaired repo-owned hooks immediately before the resumed apply. Preserve the existing physical source layout, protected backup, bare catalog-name mapping, qualified invocation identities, frozen slimming baseline, user-owned installation state, and single-active-discovery-path invariant. Execute only the independently reviewed candidate, rollback rehearsal, and final-universal sequence inside the frozen M5 boundary. Do not depend on prior chat, deleted prompts, temporary branches, or unpublished artifacts.
...
Authorization and review: `The user authorized formal identity-contract evolution on 2026-08-21, the evolved oracle and artifacts completed independent cutover review, and the user then explicitly resumed execution under this contract. The exact reviewed M5 apply and rollback operations are executable inside the frozen boundary. Any broader mutation is a runtime hard stop.`
...
Before marking the milestone Done, run its minimal falsifying checks, required broader validation, independent read-only Contract review, docs synchronization, and checkpoint evidence. M5's evolved identity/source-locator oracle has passed review and the user explicitly resumed execution; perform only its frozen apply and rollback sequence. Do not delete retained plugin, marketplace, cache, hook-backup, or Watcher state; cleanup belongs to a later independent goal.
```
