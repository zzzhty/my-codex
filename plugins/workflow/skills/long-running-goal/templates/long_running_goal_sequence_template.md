...
1. Create each child from `long_running_goal_template.md`, complete its own `grill-with-docs` preflight, and freeze its scope, owner, compatibility, validation, rollback, permission, release/deploy, privacy, and non-goal boundaries.
...
4. Run `check_goal_sequence.py --allow-draft` while drafting and without that flag before authorization or execution. Draft mode never relaxes preflight.
5. Create only one active Codex system goal for this parent. Never create nested system goals for children.
...
4. Parent authorization permits automatic handoff only inside child-frozen boundaries and never expands release, deploy, destructive, privacy-sensitive, external-write, or externally visible permission.
...
1. Pre-approved YOLO local operations: `<Only planned non-destructive local code/docs edits, tests, lint, formatting, rebuilds, refreshes, reinstalls, link checks, plugin/cache refreshes, and generated-artifact cleanup inside each child boundary.>`
...
4. Non-stops: `<M0, child handoff after passed gates, review/checkpoint boundaries, evidence recording, rebuild/refresh/reinstall, docs sync, and locally repairable validation failures.>`
...
10. Close only after every child is Closed, integration is Done, close evidence passes, current docs are synchronized, and active navigation is clean.
...
5. Final rollback, residual risk, harness conclusion, and checkpoint revision are recorded.

Close execution evidence: `<changed artifacts, final behavior, validation commands/results, docs sync, rollback, residual risk, and archive outcome>`
