...
def main() -> int:
    ...
    try:
        ...
        print(f"failed to read summary JSON: {args.input}: {exc}", file=sys.stderr)
        return 1

    if not isinstance(data, dict):
        print("summary JSON root must be an object", file=sys.stderr)
        return 1
    sections = data.get("sections")
    if not isinstance(sections, list) or not sections:
        print("summary JSON must include a non-empty sections list", file=sys.stderr)
        return 1

    title = str(data.get("title") or "Developer Summary")
    data.setdefault("generated_at", datetime.now(timezone.utc).isoformat())
    nav, sections_html = render_sections(sections)
